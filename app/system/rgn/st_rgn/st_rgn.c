...
MI_S32 ST_OSD_Init(void)
{
    ...
    if (g_bInit == FALSE)
    {
        ...
        ExecFunc(MI_RGN_Init(0, &g_stPaletteTable), MI_RGN_OK);
...
MI_S32 ST_OSD_Deinit(void)
{
    ...
    if (g_bInit == TRUE)
    {
        ...
        ExecFunc(MI_RGN_DeInit(0), MI_RGN_OK);
        // MI_RGN_Init(&stPaletteTable);
    }
    pthread_mutex_unlock(&g_rgnOsd_mutex);
...
{
    ExecFunc(MI_RGN_Create(0, hHandle, pstRegion), MI_RGN_OK);
...
{
    ExecFunc(MI_RGN_Destroy(0, hHandle), MI_RGN_OK);
