...
u32 vts_30fps;    //1125//1090                              //for 29.091fps @ MCLK=36MHz
#if HDR_DOL_10bit
...
const static I2C_ARRAY  Sensor_init_table_4lane_60P[] =
{
    {0x3002, 0x01},   //Master mode stop
    {0xffff, 0x14},//delay
    {0x3000, 0x01},   // standby
...
static int pCus_init_mipi4lane_linear_60fps(ms_cus_sensor *handle)
{
    ...
    //UartSendTrace("IMX307 Sensor_init_table_4lane\n");
    for(i=0;i< ARRAY_SIZE(Sensor_init_table_4lane_60P);i++)
    {
        if(Sensor_init_table_4lane_60P[i].reg==0xffff)
        {
            SENSOR_MSLEEP(Sensor_init_table_4lane_60P[i].data);
        }
        else
        {
            cnt = 0;
            while(SensorReg_Write(Sensor_init_table_4lane_60P[i].reg,Sensor_init_table_4lane_60P[i].data) != SUCCESS)
            {
                cnt++;
                //printf("Sensor_init_table_4lane -> Retry %d...\n",cnt);
                if(cnt>=10)
...
static int pCus_SetFPS(ms_cus_sensor *handle, u32 fps)
{
	...
	//printk("[%s] senser_set_FPS %d vts=0x%x.\n",__FUNCTION__,fps,vts); //ouyang

   // params->expo.vts = vts;   //ouyang
...
static int pCus_SetAEUSecs(ms_cus_sensor *handle, u32 us)
{
	...
	//printk("[%s] us %d, line=0x %x, vts =0x%x\n",__FUNCTION__,us, lines,params->expo.vts); //ouyang
