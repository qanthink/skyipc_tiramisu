...
{
    AF_ROI_MODE_TYPE_e eMode;
    MI_U32 u32VerticalBlockNumber;
} MI_ISP_AF_HW_ROI_MODE_TYPE_t;

typedef struct AF_WIN_TYPE_s
{
    MI_U16 u16StartX;        /*range : 0~1023*/
    MI_U16 u16StartY;        /*range : 0~1023*/
    MI_U16 u16EndX;          /*range : 0~1023*/
    MI_U16 u16EndY;          /*range : 0~1023*/
} AF_WIN_TYPE_t;

typedef struct MI_ISP_AF_HW_WIN_TYPE_s
{
    AF_WIN_TYPE_t stParaAPI[16];
...
typedef struct MI_ISP_AF_HW_YPARAM_TYPE_s
{
    MI_U8 u8R;
    MI_U8 u8G;
    MI_U8 u8B;
} MI_ISP_AF_HW_YPARAM_TYPE_t;

typedef enum __attribute__ ((aligned (1)))
{
    IQ_AF_SOURCE_FROM_SE_OBC_BF_HDR = 2,
    IQ_AF_SOURCE_FROM_SE_WBG_BF_HDR = 3,
    IQ_AF_SOURCE_FROM_ALSC_AF_HDR  = 4,
    IQ_AF_SOURCE_FROM_WBG_AF_HDR = 5,
    IQ_AF_SOURCE_FROM_LE_OBC_BF_HDR = 6,
    IQ_AF_SOURCE_FROM_LE_WBG_BF_HDR = 7,
} MI_ISP_AF_HW_SOURCE_TYPE_e;

typedef struct MI_ISP_AF_HW_PREFILTER_TYPE_s
{
    MI_U8 u8IIR1En;
...
} MI_ISP_AF_HW_PREFILTER_TYPE_t;

typedef struct MI_ISP_AF_HW_YMAP_TYPE_s
{
    MI_U8 u8En;
    MI_U8 u8LumaSource;  //0: bef ymap; 1: aft ymap
    MI_U8 u8YMapX[8];    // 1~9
    MI_U16 u16YMapY[9];  // 0~1023
} MI_ISP_AF_HW_YMAP_TYPE_t;

typedef struct MI_ISP_AF_HW_LDG_TYPE_s
{
    MI_U8 u8IIR1En;
    MI_U8 u8IIR2En;
    MI_U8 u8FIRHEn;
    MI_U8 u8FIRVEn;
    MI_U16 u16IIRCurveX[6];    // 0~1023
    MI_U8  u8IIRCurveY[6];    // 0~255
    MI_U16 u16FIRCurveX[6];    // 0~1023
    MI_U8  u8FIRCurveY[6];    // 0~255
} MI_ISP_AF_HW_LDG_TYPE_t;
...
typedef enum __attribute__ ((aligned (4)))
{
    eWDRCurveFull_GlobalTone,
    eWDRCurveFull_Curve1,
    eWDRCurveFull_Curve2,
    eWDRCurveFull_MAX
...
typedef struct WDRCurveFull_PARAM_s
{
    ...
    MI_U16 u16Curve1[33];          //0~4096
    MI_U16 u16Curve2[33];          //0~4096
...
{
    MI_ISP_BOOL_e bEnable[eWDRCurveFull_MAX];      //0 ~ 1
    MI_ISP_OP_TYPE_e enOpType[eWDRCurveFull_MAX];  //M_AUTO ~ (M_MODMAX-1)
